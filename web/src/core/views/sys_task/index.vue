<template>
  <el-card class="container" shadow="never" v-loading="pageLoading">
    ...
    <el-table :data="tableData"  @selection-change="handleSelectionChange">
      ...
      <el-table-column label="允许并发" prop="allow_concurrency">
        <template #default="{ row }">
    ...
    <FormDialog v-model="dialogOpen" v-model:form="submitForm" :title="`${ submitForm.id ? '修改' : '新增' }任务管理`" :on-confirm="handleSubmit">
      ...
      <el-form-item label="允许并发" prop="allow_concurrency">
        <el-switch v-model="submitForm.allow_concurrency" :active-value="true" :inactive-value="false"></el-switch>
      </el-form-item>
...
import { SysTaskApi } from "../../apis/sys_task.ts";
...
const getHandlers = async () => {
  const response = await SysTaskApi.GetRegisteredHandler()
...
const getPageData = async () => {
  ...
  try {
    const response = await SysTaskApi.List(queryForm.value)
...
const handleSubmit = async ()=>{
  if (!submitForm.value.id){
    await SysTaskApi.Create(submitForm.value)
    ElMessage.success("创建成功")
  }else{
    await SysTaskApi.Edit(submitForm.value)
