...
func (s *serviceStruck) List() (list []*SysRole, total int64, err error) {
	err = state.DB().Model(SysRole{}).Find(&list).Error
	return
}

func (s *serviceStruck) Create(data *SysRole) (err error) {
	err = state.DB().Create(data).Error
	return
}
...
func (s *serviceStruck) DeleteByIds(req *request2.ReqIds) error {
	return state.DB().Transaction(func(tx *gorm.DB) error {
		err := req.WithQuery(state.DB()).Where("`default` = ?", false).Delete(&SysRole{}).Error
		if err != nil {
...
func (s *serviceStruck) UpdatePermission(data *SysRole) (err error) {
	...
	return state.DB().Model(data).Association("Menus").Replace(data.Menus)
}
